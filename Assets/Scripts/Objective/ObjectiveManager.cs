...
namespace SG{
    ...
    public class ObjectiveManager : MonoBehaviour
    {
        ...
        {
            return new List<ObjectiveData>(Objective_Dict.Values);
        }

        public void SetObjectiveData(List<ObjectiveData> data)
        {
            foreach (var objectiveData in data)
            {
                if (Objective_Dict.ContainsKey(objectiveData.objectiveName))
                {
                    // Update existing objectives
                    var objective = Objective_Dict[objectiveData.objectiveName];
                    objective.objectiveDescription = objectiveData.objectiveDescription;
                    objective.objectiveComplete = objectiveData.objectiveComplete;
                }
                else
                {
                    // Add new objectives
                    Objective_Dict[objectiveData.objectiveName] = new ObjectiveData(
                        objectiveData.objectiveName,
                        objectiveData.objectiveDescription
                    )
                    {
                        objectiveComplete = objectiveData.objectiveComplete
                    };
                }
            }
